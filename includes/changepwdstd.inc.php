...
if (isset($_POST['chgpwdstd-submit'])) {
  ...
  if (empty($oldPassword) || empty($newPassword) || empty($passwordRepeat)) {
    header("Location: ../pages/pwdchangestd.php?error=emptyfields");
    exit();
  } else if ($newPassword !== $passwordRepeat) {
    header("Location: ../pages/pwdchangestd.php?error=passwordcheck");
    exit();
  } else {
      $sql = "SELECT Password FROM student_employee WHERE WS_ID=?";
      $stmt = mysqli_stmt_init($conn);
      if (!mysqli_stmt_prepare($stmt,$sql)) {
        header("Location: ../pages/pwdchangestd.php?error=sqlerror");
        exit();
      } else {
        $pwdCheck = password_verify($oldPassword, $passwordCheck);
        if ($pwdCheck == false) {
          header("Location: ../pages/pwdchangestd.php?error=pwdwrong");
        } else {
