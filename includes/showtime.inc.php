...
if (isset($_SESSION['student-user'])){
  if(empty($_SESSION['timeInRows']) == 0){
    echo "<form action='chg-time.php' method='post'>";
    echo "<table>";
      echo "<tr>";
      echo "<th>Select</th>";
      ...
      if (!isset($_SESSION['timeOutRows'])) {
        printf ('<tr> <td><input type="checkbox" name="chg-time" value="0"></td> <td>%s</td> <td>%s</td> <td></td> <td></td> <td></td> </tr>',
        ...
        if (count($_SESSION['timeOutRows']) !== count($_SESSION['timeInRows'])){
          ...
          foreach($_SESSION['timeOutRows'] as $row) {
            ...
            printf ('<tr> <td><input type="checkbox" name="chg-time" value="'.$i.'"></td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> </tr>',
        ...
        printf ('<tr> <td><input type="checkbox" name="chg-time" value="'.$i.'"></td> <td>%s</td> <td>%s</td> <td></td> <td></td> <td></td> </tr>',
        $_SESSION['timeInRows'][$i][0], $_SESSION['timeInRows'][$i][1]);

        if ($totalMin/60 > 1) {
          $totalHr += floor($totalMin/60);
          $totalMin = $totalMin % 60;
          echo ("<h4>Total Time : Hours ".$totalHr." Minutes ".$totalMin."</h4>");
        } else {
          echo ("<h4>Total Time : Hours ".$totalHr." Minutes ".$totalMin."</h4>");
        }
